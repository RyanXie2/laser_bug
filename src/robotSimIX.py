...
from numpy import asfarray, dot, c_, mean, exp, sqrt, any, isnan, asarray, array
from numpy.linalg import svd
from numpy.random import randn

import rclpy
from rclpy.node import Node
from pub_sub import RobPosSubscriber
...
class RobotSimInterface( object ):
  ...
  def getTagMsg( self ):
    ...
    #subscibe and get the robot position
    rclpy.init()
    rsub = RobPosSubscriber()
    rclpy.spin_once(rsub)
    # robot_x = round(rsub.x, 2)
    # robot_y = round(rsub.y ,2)
    robot_x = 1374.0
    robot_y = 1278.0
    ...
    self.tagPos = array([[robot_x +20, robot_y], [robot_x-20, robot_y], [robot_x, robot_y+20], [robot_x, robot_y-20]])
    
    state = { ROBOT_TAGID[0] : self.tagPos }
    state.update( self.waypoints )
    print("Getting robot position and waypoints")
    print(self.tagPos)
    # print(self.waypoints)
    # Combine all into a list
    msg = tags2list(state) + self._msg + self._lw + self._lr
    # print("State in list form")
    #print(tags2list(msg))
    # print("Printing random stuff after tag position and waypoints")
    # print(self._msg)
    # print(self._lw)
    # print(self._lr)
    # Serialize
